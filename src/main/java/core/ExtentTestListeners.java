...
public class ExtentTestListeners implements ITestListener, ISuiteListener {
    ...
    public void onTestSuccess(ITestResult result) {
        ...
        test.get().pass(CustomAssertion.getAssertMessages().toString());
        //test.get().getModel().setStatus(Status.PASS);
        getRunningReport().flush();
    }
    ...
    public void onTestFailure(ITestResult result) {
        //System.out.println("\nON TEST FAIL");
        addScreenShotToReport();
        // Passando o Fail como detalhe e não direto pelo Throwable, tem como fazer a mensagem ficar simplificada e não ficar cagada
    ...
    /**
     * Adiciona o print ao report
     */
    public void addScreenShotToReport() {
        //TODO Com o fork do extent, implementar screeshot por base64
        try {
            getInstance().addScreenCaptureFromPath("screenshot.png");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
